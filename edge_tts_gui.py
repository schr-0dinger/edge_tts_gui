...
class App(ctk.CTk):
    ...
    # Show Warning message at startup if not connected to internet
    ...
    # Messagebox for empty Textbox
    def show_empty(self):
        CTkMessagebox(title="Warning Message!", message="Text box cannot be empty", icon="warning", option_1="OK")
        self.btn_preview.configure(state="normal")  

    async def convert(self, TEXT, VOICE, OUTPUT_FILE, RATE, PITCH, VOLUME, FORMAT): # async function
        # Yet, by default, edge-tts has no WAV output
        communicate = edge_tts.Communicate(TEXT, VOICE, rate=RATE, pitch=PITCH, volume=VOLUME)
        await communicate.save(OUTPUT_FILE)             # save output file as MP3 (default)
        if FORMAT == "WAV":                             # if the format selected is WAV, 
            sound = AudioSegment.from_mp3(OUTPUT_FILE)  # convert MP3 to WAV
            sound.export(f"{OUTPUT_FILE}.wav", format="wav")
            os.remove(OUTPUT_FILE)

    async def preview(self, TEXT, VOICE, OUTPUT_FILE, RATE, PITCH, VOLUME): # async function
        communicate = edge_tts.Communicate(TEXT, VOICE, rate=RATE, pitch=PITCH, volume=VOLUME)
    ...
    """ Starting conversion and preview on separate threads """
    ...
    # Reset to default values of the scales
